...
próximo $\leftarrow$ A
dif $\leftarrow pj - próximo + 1$

\While{$dif > 0$}{
	\If{último dígito de próximo es cero y puedo restarle 10^{cantidad de ceros a derecha eliminados} a dif sin que quede negativo}{
		próximo $\leftarrow$ $[próximo / 10]$
	}
	\Else{\If{no puedo restarle 10^{cantidad de ceros a derecha eliminados} a dif sin que quede negativo}{
		próximo $\leftarrow$ $próximo \times 10$
	}}

    cerosNuevo $\leftarrow$ cantidad de ceros a izquierda del prefijo A - digitos(próximo) - cantidad de ceros a derecha eliminados
